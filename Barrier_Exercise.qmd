...
mesh <- make_mesh(dat, xy_cols = c("X", "Y"), cutoff = 0.75)
...
Build the prediction grid from the emersion-time raster (so every prediction cell has a real `dvd` value), and predict from each model. I remove subtidal area from prediction grid, as the survey is intertidal.
